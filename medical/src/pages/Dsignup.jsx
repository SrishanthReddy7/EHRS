...
export default function DSignupForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (!firstName || !lastName || !uid || !aadhaar || !email || !password || !confirmPassword) {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/users", {
        ...
        body: JSON.stringify({
          ...
          age: 30, // temporary non-zero default to satisfy backend validation
      ...
      <div className="bg-white/5 border border-white/10 text-slate-100 p-8 md:p-10 rounded-3xl shadow-lg max-w-5xl w-full flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-2/3 md:pr-8">
          ...
          <p className="text-sm text-slate-200/80 mb-4 text-center md:text-left">
            Already have an account? <Link to="/login" className="text-teal-300 hover:underline font-semibold">Login</Link>
          </p>
